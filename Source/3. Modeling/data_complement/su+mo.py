...
for region in regions:
    ...
    for file in X_raw[region].keys():
        ...
        # 정규화된 데이터를 1차원 배열로 변환하여 리스트에 추가
        X_complement.append(normalized_data.flatten())

    print(len(X_complement))

    # Prepare the train_y labels for the four categories
    y_memberNo_complement = []
    y_jobType_complement = []
    y_houseType_complement = []
    y_houseArea_complement = []

    for file in y_raw[region].keys():
        # Append the label for each category to its respective list
        y_memberNo_complement.append(y_raw[region][file].query("type == 'memberNo'")['value'].iloc[0])
        y_jobType_complement.append(y_raw[region][file].query("type == 'jobType'")['value'].iloc[0])
        y_houseType_complement.append(y_raw[region][file].query("type == 'houseType'")['value'].iloc[0])
        y_houseArea_complement.append(y_raw[region][file].query("type == 'houseArea'")['value'].iloc[0])

    for X, y in zip(X_complement, y_memberNo_complement):
        if y == 1:
            X_add.append(X)
            y_add.append(y)

X = np.array(X_origin+X_add)
target_y = np.array(y_memberNo+y_add)
# %%
import torch
...
axs[0].pie(train_y_count, labels=[1, 2, 3], textprops=fontdict_labels, colors=colors, autopct=make_autopct(train_y_count), wedgeprops=dict(width=0.7))
axs[0].set_title('Train', fontdict=fontdict_labels)

axs[1].pie(valid_y_count, labels=[1, 2, 3], textprops=fontdict_labels, colors=colors, autopct=make_autopct(valid_y_count), wedgeprops=dict(width=0.7))
axs[1].set_title('Valid', fontdict=fontdict_labels)

axs[2].pie([307, 348, 334], labels=[1, 2, 3], textprops=fontdict_labels, colors=colors, autopct=make_autopct([307, 348, 334]), wedgeprops=dict(width=0.7))
axs[2].set_title('Train', fontdict=fontdict_labels)
...
for epoch in range(_epoch):
    ...
    if np.mean(valid_loss_epoch) < best_val_loss:
        ...
        torch.save(model.state_dict(), f'model/{time}.pth')
...
X_load = np.load('/home/user/Documents/oy6uns/Source/3. Modeling/data_complement/Test_set/X_test.npy')
y_load = np.load('/home/user/Documents/oy6uns/Source/3. Modeling/data_complement/Test_set/y_test.npy')

X = torch.from_numpy(X_load).to(device)
y = torch.from_numpy(y_load).to(device)
...
memberNo_count = [labels.count(1), labels.count(2), labels.count(3)]
fig, axs = plt.subplots(4, 1, figsize=(5, 20))

colors = ['#ff9999','#66b3ff','#99ff99']

axs[0].pie(memberNo_count, labels = [1, 2, 3], textprops=fontdict_labels, colors = colors, autopct = '%1.1f%%', wedgeprops=dict(width=0.7))
axs[0].set_title('memberNo', fontdict=fontdict_labels)

axs[1].pie(memberNo_count, labels = [1, 2, 3], textprops=fontdict_labels, colors = colors, autopct = '%1.1f%%', wedgeprops=dict(width=0.7))
axs[1].set_title('jobType', fontdict=fontdict_labels)

axs[2].pie(memberNo_count, labels = [1, 2, 3], textprops=fontdict_labels, colors = colors, autopct = '%1.1f%%', wedgeprops=dict(width=0.7))
axs[2].set_title('houseArea', fontdict=fontdict_labels)

axs[3].pie(memberNo_count, labels = [1, 2, 3], textprops=fontdict_labels, colors = colors, autopct = '%1.1f%%', wedgeprops=dict(width=0.7))
