...
for file in X_raw[region].keys():
    ...
    for t in dict.keys():
        sum = 0
        count = 0
        for i in range(t, len(data.flatten()), 24):
            count += 1
            dict[t].append(round(data.flatten()[i], 2))
            sum += data.flatten()[i]
...
for i, (key, data) in enumerate(dict.items()):
    ...
    ax = axes[row, col]
    ax.hist(data, bins=30)  # 각 subplot에 히스토그램 그리기
...
np.save('/home/user/Documents/oy6uns/Source/3. Modeling/data_complement/Test_set/X_test.npy', X_test)
np.save('/home/user/Documents/oy6uns/Source/3. Modeling/data_complement/Test_set/y_test.npy', y_test)
