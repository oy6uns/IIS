...
for region in X_raw.keys():
    ...
    for file in X_raw[region].keys():
        ...
        for i in range(1, 365):
            new_data.iloc[i, 0] = data.iloc[i, 0] - data.iloc[i - 1, 23]

        # 나머지 열의 처리
        for i in range(365):
            for j in range(1, 24):
                new_data.iloc[i, j] = data.iloc[i, j] - data.iloc[i, j - 1]

        # 데이터 추가
        all_data_region = pd.concat([all_data_region, new_data], ignore_index=True)
    ...
    if scaler_num == 0:
        ...
        q1, q50, q97 = np.percentile(data_1d, [25, 50, 75])
