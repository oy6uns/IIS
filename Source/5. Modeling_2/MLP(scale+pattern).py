...
for region in X_raw.keys():
    ...
    for file in X_raw[region].keys():
        ...
        for i in range(1, 365):
            new_data.iloc[i, 0] = round(data.iloc[i, 0] - data.iloc[i - 1, 23] / data.iloc[i - 1, 23], 2)

        # 나머지 열의 처리
        for i in range(365):
            for j in range(1, 24):
                new_data.iloc[i, j] = round(data.iloc[i, j] - data.iloc[i, j - 1] / data.iloc[i, j - 1], 2)
    ...
    if scaler_num == 0:
        ...
        # 스케일링 적용
        scaled_data_region = (bounded_data - mean) / (max_val - min_val)
