...
for region in X_raw.keys():
    ...
    if scaler_num == 0:
        ...
        q1, q50, q97 = np.percentile(data_1d, [0, 50, 97])

        # Robust Scaling 적용
        scaled_data_region = (all_data_region - q50) / (q97 - q1)
    else:
        if scaler_num == 4:
            scaler = RobustScaler()
        if scaler_num == 2:
...
for file in scaled_X_raw[region].keys():
    # DataFrame에서 불필요한 열을 제거하고 값들을 추출
    if y_raw[region][file].query("type == 'memberNo'")['value'].iloc[0] == 1:
        data = scaled_X_raw[region][file].values
        
        # 정규화된 데이터를 1차원 배열로 변환하여 리스트에 추가
        X_add.append(data.flatten())

        y_add_memberNo.append(y_raw[region][file].query("type == 'memberNo'")['value'].iloc[0])
        y_add_jobType.append(y_raw[region][file].query("type == 'jobType'")['value'].iloc[0])
        y_add_houseType.append(y_raw[region][file].query("type == 'houseType'")['value'].iloc[0])
        y_add_houseArea.append(y_raw[region][file].query("type == 'houseArea'")['value'].iloc[0])
