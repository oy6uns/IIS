...
for region in X_raw.keys():
    ...
    # 전체 데이터에 대한 사분위수 계산
    q0, q50, q97 = np.percentile(data_1d, [0, 50, 97])
    # std = np.std(data_1d)
    # mean = np.mean(data_1d)

    '''Robust Scaling 적용'''
    scaled_data_region = (all_data_region - q50) / (q97 - q0)

    '''Standard Scaling 적용'''
    # scaled_data_region = (all_data_region - mean) / std
...
for file in scaled_X_raw[region].keys():
    ...
    data = scaled_X_raw[region][file].iloc[:, start_time:end_time].values
    # data = scaled_X_raw[region][file].iloc[:, list(range(0, end_time)) + list(range(start_time, 24))].values
...
print(data.shape)
# %%
'''train test split 및 CustomDataset 선언'''
import random
...
# %%
'''모델 및 Optimizer, Loss Function 선언'''
...
if region == '1.suncheon':
    ...
    # criterion = FocalLoss(alpha=[1, 0.3, 0.3], gamma=3, device=device)
elif region == '2.mokpo':
...
for epoch in range(_epoch):
    ...
    if np.mean(valid_loss_epoch) < best_val_loss:
        ...
        print(f'''Model saved at epoch {epoch+1} with valid loss: {round(best_val_loss, 4)}, validation acc: {valid_epoch_accuracy}%, f1 score: {f1:.3f}''')
...
calculate_accuracy(all_predicted, all_labels)
print(f'''Model saved at epoch {best_model_epoch} with valid loss: {round(best_val_loss, 4)}, train acc: {best_model_train_accuracy:.3f}%, valid acc: {best_model_valid_accuracy:.3f}%, f1 score: {f1:.3f}''')
# %%
'''Validation 평가 Confusion Matrix'''
print(f'{start_time}시 부터 {end_time - 1}까지 학습한 모델입니다.')
make_confusion_matrix(all_predicted, all_labels)
# %%
'''Train Loss, Valid Loss 확인'''
...
# %%
'''Correct/non-Correct 라벨들 확인'''
import matplotlib.pyplot as plt
import matplotlib.colors
import numpy as np
import seaborn as sns

# Adjust labels and predicted arraysa
labels = labels + 1
